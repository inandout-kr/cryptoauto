...
for i in range(1, n+1) :
    ...
    raw_low_price = list_a[11*i -6]
    low_price = float(raw_low_price.split(":")[1])
    dict_['low'] = low_price
...
while True:
    try:
        ...
        print(current_price)
        print((list_[1]['closed']+list_[2]['closed']+list_[3]['closed'])/3)
        if current_price > (list_[1]['closed']+list_[2]['closed']+list_[3]['closed'])/3 :
            krw = get_balance("KRW")
            if krw > 5050:
        ...
        elif current_price < ((list_[1]['closed']+list_[2]['closed']+list_[3]['closed'])/3)+50:
            eos = get_balance('EOS')
            upbit.sell_market_order("KRW-EOS", eos*0.9995)
            sell_result = upbit.sell_market_order("KRW-EOS", eos*0.9995)
            post_message(myToken,"#crypto", "EOS sell : " +str(sell_result))
        else:
            print("nothing")
        time.sleep(1)
